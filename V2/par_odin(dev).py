...
def calcul(G, active):
    ...
    if mage:
        ...
        if active:
            print("force",G[mage]["type"]," =", G[mage]["force"])
    if active:
    ...
    for k in G:
        ...
        if name in l_blanc:
            ...
            if active:
                print("[F.",k["type"],"] t = ",t, " (",k["force"],"x", t_in,")", sep='')
    ...
    if l_strong:
        ...
        for k in G:
            ...
            if name == "Fenrir" and l_strong:
                t += stronger
                if active:
                    print("[Fenir] t =", t, "(+", stronger, ")")
            elif name == "Jormungand" and l_strong:
                t -= stronger*2
                if active:
                    print("[Jormungand] t = ", t, " (-", stronger, "*2)", sep='')
            elif name == "Sleipnir":
                t += t_blanc+t_mage
                if active:
                    print("[Sleipnir] t = ", t, " (+", t_blanc+t_mage, ")", sep='')
            elif name == "Fafnir":
                t -= t_blanc+t_mage
                if active:
                    print("[Fafnir] t = ", t, " (-", t_blanc+t_mage, ")", sep='')
            elif l_identique:
                if name == "Gullinbursti":
                    plus = l_identique[randint(0, len(l_identique)-1)]
                    t += plus
                    if active:
                        print("[Gullinbursti] t = ", t, " (+", plus, ")", sep='')
                elif name == "Hraesvelg":
                    plus = l_identique[randint(0, len(l_identique)-1)]
                    t -= plus
                    if active:
                        print("[Hraesvelg] t = ", t, " (-", plus, ")", sep='')
    return t
